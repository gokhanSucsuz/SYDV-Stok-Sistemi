...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if (!id) throw new Error("İşlem (Transaction) ID gereklidir.");
    ...
    if (Date.now() - transaction.date > tenMinutes) {
      throw new Error("İşlem üzerinden 10 dakikadan fazla geçtiği için geri alınamaz.");
    }

    const item = await Item.findById(transaction.itemId).session(session);
    if (!item) throw new Error("İşleme ait malzeme bulunamadı.");

    if (transaction.type === "GİRİŞ") {
      item.currentStock -= transaction.quantity;
      if (item.totalReceived !== undefined) {
        item.totalReceived -= transaction.quantity;
      }
    } else if (transaction.type === "ÇIKIŞ") {
      item.currentStock += transaction.quantity;
    }
