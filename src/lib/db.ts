...
export async function addPersonnel(
  personnel: Omit<Personnel, "id" | "createdAt">,
): Promise<string> {
  const res = await fetch("/api/personnel", {
    method: "POST",
...
export async function getTransactionsByUnit(
  unit: UnitType,
): Promise<Transaction[]> {
...
export async function addTransaction(
  tx: Omit<Transaction, "id" | "remainingStock">,
): Promise<string> {
  const res = await fetch("/api/transactions", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function addMasterItem(
  item: Omit<MasterItem, "id" | "createdAt">,
): Promise<string> {
  const res = await fetch("/api/master-items", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function bulkAddMasterItems(
  items: Omit<MasterItem, "id" | "createdAt">[],
): Promise<void> {
  const res = await fetch("/api/master-items", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function checkDocumentNoExists(
  documentNo: string,
): Promise<boolean> {
  const res = await fetch(
    `/api/transactions?documentNo=${encodeURIComponent(documentNo)}`,
  );
